...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The model name for Q&A generation.",
    )
...
def generate_question_answer(line: str, model: str) -> List[QA]:
    """
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            try:
                # qa_response = QAResponse.parse_raw(json_str)
...
def main():
    ...
    output_dir = os.path.join(model_name)  # Use model name as the output directory
    os.makedirs(output_dir, exist_ok=True)

    # Define output file paths inside the model name directory
    FINAL_OUTPUT_FILE = os.path.join(
        output_dir, f"questions_answers_temp-{model_name}.json"
    )
    ...
    # Load progress and matrix state
            ...
            with open(filepath, "r", encoding="utf-8") as infile:
                for line_number, line in enumerate(infile, start=1):
                    ...
                    qa_pairs, total_tokens, completion_tokens, prompt_tokens = (
                        generate_question_answer(line, model_name)
                    )

                    if not qa_pairs:
                        logging.warning(
                            f"No Q&A pairs generated for line {line_number} in {filename}"
                        )
                        continue
                    ...
                    # Write partial results after each line
    ...
    matrix["total_time"] = total_time  # Save execution time to the matrix
    save_matrix(matrix, MATRIX_FILE)  # Save the matrix to the model directory
